...
float calculate_range(float position = 0) {
    ...
    int closest_index = find_closest_index(m430, position, (sizeof(m430)/sizeof(m430[0])));

    Serial.print("Closest index = ");
    Serial.println(closest_index);
    Serial.print("Closest range = ");
    Serial.println(ranges[closest_index]);
